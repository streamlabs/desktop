...
export type TTimeInputProps = TSlobsInputProps<{ isTimer?: boolean }, number, TimePickerProps>;

export const TimeInput = InputComponent((p: TTimeInputProps) => {
  const { wrapperAttrs, inputAttrs } = useInput('time', p);
  const value = getAntdValue(inputAttrs.value, p.isTimer);
    ...
    const val = isParsed
      ...
      : // If the moment is not parsed, it's a user-inputted time, so we need to preserve the original date
        value
          ...
          .set('seconds', p.isTimer ? moment.seconds() : 0);
  ...
  return (
    <InputWrapper {...wrapperAttrs}>
      <TimePicker
        ...
        onSelect={onChange}
        onChange={onChange}
        use12Hours={!p.isTimer}
        format={p.isTimer ? 'mm:ss' : 'h:mm a'}
        showNow={false}
        allowClear={false}
      />
    </InputWrapper>
...
function getAntdValue(value?: number, isTimer?: boolean) {
  if (isTimer) return moment({ seconds: value });
  return moment(value);
}
