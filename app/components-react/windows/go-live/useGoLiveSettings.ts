...
class GoLiveSettingsState extends StreamInfoView<IGoLiveSettingsState> {
  ...
  updatePlatform(platform: TPlatform, patch: Partial<IGoLiveSettings['platforms'][TPlatform]>) {
    // A scheduled broadcast from YouTube has its own title and description, so  mark YouTube as using
    // custom fields. Without this the shared title is applied over the scheduled one and going live
    // renames the event. This must be handled when updating the platform, not when prepopulating the
    // go live window or getting the platform's saved settings, because the scheduled stream is only
    // selected after the user has opened the go live window.
    const platformPatch =
