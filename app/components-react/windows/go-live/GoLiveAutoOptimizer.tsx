...
import { AutoOptimizerFlow } from 'components-react/shared/auto-optimizer';
...
const estimateReasonLabels: Record<string, string> = {
  ...
  probe_disabled: 'Estimated because the Twitch bandwidth test was unavailable.',
  probe_failed: 'Estimated because the Twitch bandwidth test could not be completed.',
...
export default function GoLiveAutoOptimizer() {
  ...
  const state = useVuex(() => {
    ...
    const { stage, phase, progress, topology, result, error } = service.state;
    return { stage, phase, progress, topology, result, error };
  });
  ...
  const phaseLabel = state.phase
    ? {
        ...
        bandwidth: state.topology?.activeBandwidthTest
          ? $t('Measuring your Twitch upload...')
          : $t('Estimating safe upload settings...'),
  ...
  const legs = (state.result?.legs || []).map(leg => ({
    ...
    estimateReason: leg.estimateReason
      ...
      : undefined,
    managedByProvider: leg.display === 'both' || state.result?.topology === 'enhanced-broadcasting',
