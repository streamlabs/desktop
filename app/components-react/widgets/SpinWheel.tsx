...
export function SpinWheel() {
  ...
  return (
    <WidgetLayout>
      ...
      <Form>
        {!w.state.isLoading && w.settings && w.selectedTab === 'wheel' && (
          <FormFactory metadata={w.wheelMeta} values={mutableSettings} onChange={w.updateSetting} />
        )}
        {!w.state.isLoading && w.settings && w.selectedTab === 'categories' && (
          <CategoryEditor
            categories={w.settings.categories}
            onAdd={w.addCategory}
            onClear={w.clearCategories}
            onEdit={w.editCategory}
            onRemove={w.removeCategory}
          />
        )}
        {!w.state.isLoading && w.settings && w.selectedTab === 'sections' && (
          <SectionEditor
            sections={w.settings.sections}
            sectionOptions={w.sectionOptions}
            addSectionDisabled={w.settings.categories.length === 0}
            onAdd={w.addSection}
            onClear={w.clearSections}
            onEdit={w.editSection}
            onRemove={w.removeSection}
            onMove={w.moveSection}
          />
        )}
        ...
        {!w.state.isLoading && w.settings && w.selectedTab === 'border' && (
          <FormFactory
            metadata={w.borderMeta}
            values={mutableSettings}
            onChange={w.updateSetting}
          />
        )}
        {!w.state.isLoading && w.settings && w.selectedTab === 'ticker' && (
          <FormFactory
            metadata={w.tickerMeta}
            values={mutableSettings}
            onChange={w.updateSetting}
          />
        )}
        {!w.state.isLoading && w.settings && w.selectedTab === 'image' && (
          <FormFactory metadata={w.imageMeta} values={mutableSettings} onChange={w.updateSetting} />
        )}
      </Form>
    </WidgetLayout>
  );
