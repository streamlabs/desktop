...
export function Jar() {
  const w = useJar() as ReturnType<typeof useJar> & { staticConfig?: IJarWidgetStaticConfig };

  // This value is used in the meta to build the type/tier lists. Do not use it in the factory.
  const mutableSettings = w.settings as Omit<NonNullable<typeof w.settings>, 'types'>;
  ...
  get jarMeta() {
    ...
    return fromMeta({
      ...
      ...(this.settings.types.twitch_bits
        ...
        : {}),
      ...(this.settings.types.twitch_power_ups
        ? {
            twitch_power_ups_minimum_amount: metadata.number({
              label: $t('Minimum Power-Ups'),
              min: 1,
            }),
          }
        : {}),
