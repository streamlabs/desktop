    ...
    <WidgetLayout>
      ...
      <Form>
        {w.hasLoadedSettings() && w.selectedTab === 'goal' && hasGoal && (
          <BossDisplay goal={w.goalSettings} resetGoal={w.resetGoal} />
        )}
        {w.hasLoadedSettings() && w.selectedTab === 'goal' && !hasGoal && (
          <>
            <FormFactory
              metadata={w.bossGoalMeta}
              values={bossCreateValues}
              onChange={updateBossCreate}
            />
            <FormFactory
              metadata={w.bossModeMeta(mode)}
              values={w.settings}
              onChange={w.updateSetting}
            />
            <Button
              className="button button--action"
              onClick={() => w.saveGoal(bossCreateValues)}
        ...
        {w.hasLoadedSettings() && w.selectedTab === 'general' && (
          <>
            <FormFactory metadata={w.bossMeta} values={w.settings} onChange={w.updateSetting} />
            <FormFactory
              metadata={w.multipliersMeta}
              values={w.settings}
              onChange={w.updateSetting}
            />
          </>
