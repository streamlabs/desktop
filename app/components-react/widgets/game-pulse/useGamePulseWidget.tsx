  ...
  export class GamePulseModule extends WidgetModule<IGamePulseWidgetState> {
    ...
    public toggleScope(scopeId: string, enabled: boolean) {
      const newSettings = { ...this.settings } as GamePulseWidgetSettings;
      
      const currentGroup = this.getScope(scopeId) || { triggers: [] };
    ...
    public async deleteTrigger(triggerId: string, scopeId: string) {
      ...
      if (scopeId === ScopeId.Global) {
        newSettings.global = {
          ...(newSettings.global || { enabled: true, triggers: [] })
        };
        ...
        if (group) {
          newSettings.games[scopeId] = {
              ...group,
              triggers: (group.triggers || []).filter(t => t.id !== triggerId)
          };
        }
      }
    ...
    private appendTriggerToSettings(
      ...
      const nextSettings = { ...settings } as GamePulseWidgetSettings;
      ...
      if (scopeId === ScopeId.Global) {
        ...
        nextSettings.games = { ...(nextSettings.games || {}) };
        
        const group = nextSettings.games[scopeId] || {
          enabled: true,
          triggers: [],
        };

        nextSettings.games[scopeId] = {
          ...group,
    ...
    protected async fetchData() {
      ...
      try {
        const res = voicesRes as TTSLanguagesResponse;
        if (res.success && this.state.staticConfig) {
          const currentConfig = this.state.staticConfig as GamePulseStaticConfig;
          const currentOptions = currentConfig.data.options || {};
          const newConfig = {
            ...currentConfig,
            data: {
              ...currentConfig.data,
              options: {
                ...currentOptions,
                tts_voices: res.data || res,
              },
            },
          } as GamePulseStaticConfig;

          this.state.setStaticConfig(newConfig);
        }
      } catch (err) {
