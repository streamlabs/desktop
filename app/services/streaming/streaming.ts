...
export class StreamingService
  ...
  async finishStartStreaming() {
    if (this.views.useFactoryAPI) {
      await this.finishV2StartStreaming();
    } else {
    ...
    if (this.views.isDualOutputMode) {
      ...
      extraOutputs.forEach(output => {
        try {
          ...
          stream.signalHandler = this.handleV1OBSOutputSignal;
  ...
  async finishV2StartStreaming(): Promise<unknown> {
    ...
    if (recordWhenStreaming && isRecordingOffline) {
  ...
  startReplayBuffer(display: TDisplayType = 'horizontal'): void {
    ...
    const index = this.views.isDualOutputMode ? 2 : 1;
    Promise.resolve(this.createReplayBuffer(display, index));
  }
  ...
  private handleV1OBSOutputSignal(info: IOBSOutputSignalInfo) {
    ...
    if (info.type === EOBSOutputType.Streaming) {
      if (info.signal === EOBSOutputSignal.Start && shouldResolve) {
        ...
        this.SET_STREAMING_STATUS(EStreamingState.Live, 'horizontal');
        this.streamingStatusChange.next(EStreamingState.Live);
        this.clearReconnectingNotification();
      }
    }
