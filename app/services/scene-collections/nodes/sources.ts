    ...
    const promises: Promise<ISourceInfo | null>[] = this.getItems().map(source => {
      return new Promise(resolve => {
        ...
        return hotkeys.save({ sourceId: source.sourceId }).then(() => {
          ...
          if (!obsInput) {
            console.warn(
              `[SourceNode] OBS input not found for source ${source.sourceId} (${source.name}), skipping save`,
            );
            return resolve(null);
          }
          ...
          const filters = this.sourceFiltersService.views
            ...
            .map(f => {
              ...
              if (!filterInput) {
                console.warn(
                  `[SourceNode] OBS filter not found for filter ${f.name} on source ${source.sourceId} (${source.name}), skipping`,
                );
                return null;
              }
